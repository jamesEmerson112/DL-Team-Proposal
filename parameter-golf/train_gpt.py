...
class CausalSelfAttention(nn.Module):
	def __init__(self,dim,num_heads,num_kv_heads,rope_base,qk_gain_init,train_seq_len,gated_attn='none',diff_attn_enabled=False,hybridnorm_v=False,layer_idx=0):
		...
		self.diff_attn_enabled=diff_attn_enabled;self.hybridnorm_v=hybridnorm_v
		if diff_attn_enabled:lambda_init=0.8-0.6*math.exp(-0.3*max(layer_idx-1,0));self.lambda_param=nn.Parameter(torch.full((num_heads,),lambda_init,dtype=torch.float32))
	...
	def forward(self,x,v0=None,vr_alpha=0.0,prev_kv=None):
		...
		if self.hybridnorm_v:v=F.rms_norm(v,(v.size(-1),))
		...
		if prev_kv is not None:pk,pv=prev_kv;k=torch.cat([pk,k],dim=1);v=torch.cat([pv,v],dim=1)
		...
		if prev_kv is not None:return out,(cur_k,cur_v)
...
class Block(nn.Module):
	def __init__(self,dim,num_heads,num_kv_heads,mlp_mult,rope_base,qk_gain_init,train_seq_len,layer_idx=0,ln_scale=False,gated_attn='none',diff_attn_enabled=False,hybridnorm_v=False):super().__init__();self.attn_norm=RMSNorm();self.mlp_norm=RMSNorm();self.attn=CausalSelfAttention(dim,num_heads,num_kv_heads,rope_base,qk_gain_init,train_seq_len,gated_attn=gated_attn,diff_attn_enabled=diff_attn_enabled,hybridnorm_v=hybridnorm_v,layer_idx=layer_idx);self.mlp=MLP(dim,mlp_mult);self.attn_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32));self.mlp_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32));self.resid_mix=nn.Parameter(torch.stack((torch.ones(dim),torch.zeros(dim))).float());self.ln_scale_factor=1./math.sqrt(layer_idx+1)if ln_scale else 1.;self.parallel=False
	def forward(self,x,x0,v0=None,vr_alpha=0.0,prev_kv=None):
		mix=self.resid_mix.to(dtype=x.dtype);x_in=mix[0][None,None,:]*x+mix[1][None,None,:]*x0;attn_result=self.attn(self.attn_norm(x_in)*self.ln_scale_factor,v0=v0,vr_alpha=vr_alpha,prev_kv=prev_kv)
...
class GPT(nn.Module):
	def __init__(self,h):
		...
		self.num_encoder_layers=h.num_layers//2;self.num_decoder_layers=h.num_layers-self.num_encoder_layers;self.value_residual_alpha=h.value_residual_alpha;self.blocks=nn.ModuleList([Block(h.model_dim,h.num_heads,h.num_kv_heads,h.mlp_mult,h.rope_base,h.qk_gain_init,h.train_seq_len,layer_idx=i,ln_scale=h.ln_scale,gated_attn=h.gated_attn,diff_attn_enabled=h.diffattn_enabled,hybridnorm_v=h.hybridnorm_v)for i in range(h.num_layers)])
		if h.rope_dims>0:
	...
	def forward_logits(self,input_ids,kv_cache=None):
		x=self.tok_emb(input_ids);x=F.rms_norm(x,(x.size(-1),))
		if self.embed_proj is not None:x=self.embed_proj(x)
		x0=x;skips=[];vr_alpha=self.value_residual_alpha;v0=None;enc_iter=self.encoder_indices if self.looping_active else range(self.num_encoder_layers);dec_iter=self.decoder_indices if self.looping_active else range(self.num_encoder_layers,self.num_encoder_layers+self.num_decoder_layers);new_kv_cache={}
		for i in enc_iter:
			pkv=kv_cache.get(i)if kv_cache and self.blocks[i].attn.use_xsa else None
			result=self.blocks[i](x,x0,v0=v0,vr_alpha=vr_alpha,prev_kv=pkv)
			if isinstance(result,tuple) and kv_cache is not None and self.blocks[i].attn.use_xsa:x,new_kv_cache[i]=result
			elif isinstance(result,tuple):x,v0=result
			else:x=result
			skips.append(x)
		for(skip_idx,i)in enumerate(dec_iter):
			...
			result=self.blocks[i](x,x0,v0=v0,vr_alpha=vr_alpha,prev_kv=pkv)
			if isinstance(result,tuple) and kv_cache is not None and self.blocks[i].attn.use_xsa:x,new_kv_cache[i]=result
			elif isinstance(result,tuple):x,_=result
			else:x=result
		x=self.final_norm(x)
		...
		if kv_cache is not None:return logits,new_kv_cache
		return logits
...
class Muon(torch.optim.Optimizer):
	...
	def step(self,closure=None):
		...
		return loss
CONTROL_TENSOR_NAME_PATTERNS=tuple(pattern for pattern in os.environ.get('CONTROL_TENSOR_NAME_PATTERNS','attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,skip_gates,lambda_param').split(',')if pattern)
class Optimizers:
	def __init__(self,h,base_model):
		...
		token_lr=h.tied_embed_lr if h.tie_embeddings else h.embed_lr;tok_params=[{'params':[base_model.tok_emb.weight],'lr':token_lr,'base_lr':token_lr}];self.schedule_free=h.schedule_free
		if h.schedule_free:
			import schedulefree;self.optimizer_tok=schedulefree.AdamWScheduleFree(tok_params,betas=(h.beta1,h.beta2),eps=h.adam_eps,weight_decay=h.embed_wd)
		else:self.optimizer_tok=torch.optim.AdamW(tok_params,betas=(h.beta1,h.beta2),eps=h.adam_eps,weight_decay=h.embed_wd,fused=True)
		...
		if h.schedule_free:
			import schedulefree;self.optimizer_scalar=schedulefree.AdamWScheduleFree([{'params':scalar_params,'lr':h.scalar_lr,'base_lr':h.scalar_lr}],betas=(h.beta1,h.beta2),eps=h.adam_eps,weight_decay=h.adam_wd)
		else:self.optimizer_scalar=torch.optim.AdamW([{'params':scalar_params,'lr':h.scalar_lr,'base_lr':h.scalar_lr}],betas=(h.beta1,h.beta2),eps=h.adam_eps,weight_decay=h.adam_wd,fused=True)
	...
	def train_mode(self):
		if self.schedule_free:
			for opt in [self.optimizer_tok,self.optimizer_scalar]:
...
def eval_val_crossseq(h,device,val_data,base_model):
	base_model.eval();seq_len=h.eval_seq_len;context_size=seq_len-h.eval_stride;total_tokens=val_data.val_tokens.numel()-1;window_starts=[ws for ws in range(0,total_tokens,h.eval_stride)if ws+context_size<total_tokens];total_windows=len(window_starts);my_s=total_windows*h.rank//h.world_size;my_e=total_windows*(h.rank+1)//h.world_size;my_windows=window_starts[my_s:my_e];loss_sum=torch.zeros((),device=device,dtype=torch.float64);token_count=torch.zeros((),device=device,dtype=torch.float64);byte_count=torch.zeros((),device=device,dtype=torch.float64);kv_cache={}
	with torch.inference_mode():
		for ws in my_windows:
			...
			with torch.autocast(device_type='cuda',dtype=torch.bfloat16):logits,kv_cache=base_model.forward_logits(x,kv_cache=kv_cache)
			nll=F.cross_entropy(logits.reshape(-1,logits.size(-1)).float(),y.reshape(-1),reduction='none').reshape(1,wlen)
			s=0 if ws==0 else context_size;scored_nll=nll[0,s:wlen].to(torch.float64);loss_sum+=scored_nll.sum();token_count+=float(wlen-s);tgt=y[0,s:wlen];prev=x[0,s:wlen];tb=val_data.base_bytes_lut[tgt].to(torch.float64);tb+=(val_data.has_leading_space_lut[tgt]&~val_data.is_boundary_token_lut[prev]).to(torch.float64);byte_count+=tb.sum()
			kv_cache={k:(kk.detach(),vv.detach())for k,(kk,vv)in kv_cache.items()}
	if dist.is_available()and dist.is_initialized():dist.all_reduce(loss_sum,op=dist.ReduceOp.SUM);dist.all_reduce(token_count,op=dist.ReduceOp.SUM);dist.all_reduce(byte_count,op=dist.ReduceOp.SUM)
	base_model.train();return _loss_bpb(loss_sum,token_count,byte_count)
...
def train_model(h,device,val_data):
	...
	ema_state={name:t.detach().float().clone()for(name,t)in base_model.state_dict().items()};ema_decay=h.ema_decay;training_time_ms=.0;stop_after_step=None;torch.cuda.synchronize();t0=time.perf_counter();step=0;optimizers.train_mode()
	while True:
		...
		if should_validate:optimizers.eval_mode();torch.cuda.synchronize();training_time_ms+=1e3*(time.perf_counter()-t0);val_loss,val_bpb=eval_val(h,device,val_data,model);log(f"{step}/{h.iterations} val_loss: {val_loss:.4f} val_bpb: {val_bpb:.4f}");torch.cuda.synchronize();t0=time.perf_counter();optimizers.train_mode()
		if last_step:
...
def train_and_eval(h,device):
	...
	if h.cross_seq_attn and h.sliding_window_enabled:timed_eval('quantized_crossseq',eval_val_crossseq,h,device,val_data,eval_model)
