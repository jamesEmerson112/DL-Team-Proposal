...
class StructuredMLP(nn.Module):
	def __init__(self,dim,mlp_mult,rank_ratio=0.5,num_blocks=4):
		super().__init__();hidden=int(mlp_mult*dim);rank=int(rank_ratio*dim);self.num_blocks=num_blocks
		self.fc_u=CastedLinear(dim,rank,bias=False);self.fc_v=CastedLinear(rank,hidden,bias=False)
		block_in=hidden//num_blocks;block_out=dim//num_blocks
		self.proj_blocks=nn.ModuleList([CastedLinear(block_in,block_out,bias=False)for _ in range(num_blocks)])
		for b in self.proj_blocks:b._zero_init=True
	def forward(self,x):
		h=self.fc_v(self.fc_u(x));h=F.leaky_relu(h,negative_slope=.5).square()
		chunks=h.chunk(self.num_blocks,dim=-1)
		return torch.cat([self.proj_blocks[i](chunks[i])for i in range(self.num_blocks)],dim=-1)
class Block(nn.Module):
	def __init__(self,dim,num_heads,num_kv_heads,mlp_mult,rope_base,qk_gain_init,train_seq_len,layer_idx=0,ln_scale=False,gated_attn='none',structured_ffn=False,ffn_rank_ratio=0.5,ffn_num_blocks=4,peri_ln=False):super().__init__();self.attn_norm=RMSNorm();self.mlp_norm=RMSNorm();self.attn_post_norm=RMSNorm()if peri_ln else None;self.mlp_post_norm=RMSNorm()if peri_ln else None;self.attn=CausalSelfAttention(dim,num_heads,num_kv_heads,rope_base,qk_gain_init,train_seq_len,gated_attn=gated_attn);self.mlp=StructuredMLP(dim,mlp_mult,rank_ratio=ffn_rank_ratio,num_blocks=ffn_num_blocks)if structured_ffn else MLP(dim,mlp_mult);self.attn_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32));self.mlp_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32));self.resid_mix=nn.Parameter(torch.stack((torch.ones(dim),torch.zeros(dim))).float());self.ln_scale_factor=1./math.sqrt(layer_idx+1)if ln_scale else 1.;self.parallel=False
	def forward(self,x,x0,v0=None,vr_alpha=0.0):
		...
		if self.attn_post_norm is not None:attn_out=self.attn_post_norm(attn_out)
		if self.parallel:mlp_out=self.mlp(self.mlp_norm(x_in)*self.ln_scale_factor);mlp_out=self.mlp_post_norm(mlp_out)if self.mlp_post_norm is not None else mlp_out;x_out=x_in+self.attn_scale.to(dtype=x_in.dtype)[None,None,:]*attn_out+self.mlp_scale.to(dtype=x_in.dtype)[None,None,:]*mlp_out
		else:x_out=x_in+self.attn_scale.to(dtype=x_in.dtype)[None,None,:]*attn_out;mlp_out=self.mlp(self.mlp_norm(x_out)*self.ln_scale_factor);mlp_out=self.mlp_post_norm(mlp_out)if self.mlp_post_norm is not None else mlp_out;x_out=x_out+self.mlp_scale.to(dtype=x_out.dtype)[None,None,:]*mlp_out
...
class GPT(nn.Module):
	def __init__(self,h):
		...
		self.num_encoder_layers=h.num_layers//2;self.num_decoder_layers=h.num_layers-self.num_encoder_layers;self.value_residual_alpha=h.value_residual_alpha;self.blocks=nn.ModuleList([Block(h.model_dim,h.num_heads,h.num_kv_heads,h.mlp_mult,h.rope_base,h.qk_gain_init,h.train_seq_len,layer_idx=i,ln_scale=h.ln_scale,gated_attn=h.gated_attn,structured_ffn=h.structured_ffn,ffn_rank_ratio=h.ffn_rank_ratio,ffn_num_blocks=h.ffn_num_blocks,peri_ln=h.peri_ln)for i in range(h.num_layers)])
		if h.rope_dims>0:
...
def train_model(h,device,val_data):
	...
	if h.lr_warmup_frac>0:log(f"lr_warmup:enabled frac={h.lr_warmup_frac}")
	if h.structured_ffn:log(f"structured_ffn:enabled rank_ratio={h.ffn_rank_ratio} num_blocks={h.ffn_num_blocks}")
	if h.peri_ln:log(f"peri_ln:enabled")
	...
	def lr_mul(frac):
		if h.lr_warmup_frac>0 and frac<h.lr_warmup_frac:return frac/h.lr_warmup_frac
...
def train_and_eval(h,device):
	random.seed(h.seed);np.random.seed(h.seed);torch.manual_seed(h.seed);torch.cuda.manual_seed_all(h.seed);val_data=ValidationData(h,device);_n_shards=len(list(Path(h.datasets_dir).resolve().glob('fineweb_train_*.bin')));log(f"train_shards: {_n_shards}");log(f"val_tokens: {val_data.val_tokens.numel()-1}");base_model,compiled_model=train_model(h,device,val_data);torch._dynamo.reset();timed_eval('pre-quantization post-ema',eval_val,h,device,val_data,compiled_model);serialize(h,base_model,Path(__file__).read_text(encoding='utf-8'))
