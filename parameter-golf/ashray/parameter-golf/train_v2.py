...
class Hyperparameters:
    ...
    # Peri-LN (Kim et al., ICML 2025, arXiv:2502.02732). Apply RMSNorm to the
    # OUTPUT of each sublayer (attn + mlp) before the residual add, in addition
    # to the existing Pre-LN on the input. Zero new parameters (stateless
    # F.rms_norm). When disabled (default), Block.forward et al. run the exact
    # Pre-LN pipeline from rank 4.
    peri_ln_enabled = bool(int(os.environ.get("PERI_LN_ENABLED", "0")))
...
class Block(nn.Module):
    ...
    def forward(self, x, x0, q_w, k_w, v_w, out_w, up_w, down_w, cu_seqlens=None, max_seqlen=0):
        ...
        # Peri-LN: normalize sublayer output before residual add + scale.
        if self.peri_ln:
...
class GPT(nn.Module):
    def __init__(self, h):
        ...
        self.peri_ln_enabled = h.peri_ln_enabled
