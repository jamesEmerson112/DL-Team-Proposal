...
class Hyperparameters:
    ...
    # Universal-Transformer-style timestep embed for depth recurrence.
    # When enabled, a learned (num_loops+1, model_dim) tensor gives the model
    # a per-iteration signal so it knows which pass through the looped layers
    # it's on (iter 0, 1, ..., num_loops). Added to x before attn_norm inside
    # each looped layer's forward. Zero-init so v4 is bit-identical to v1 at
    # init. Active only when looping_active AND layer index is in [loop_start,
    # loop_end]. Params: (num_loops+1) * model_dim fp32 = ~6 KB for defaults.
    ts_embed_enabled = bool(int(os.environ.get("TS_EMBED_ENABLED", "0")))
...
class GPT(nn.Module):
    def __init__(self, h):
        ...
        # Universal-Transformer-style timestep embed (Dehghani et al., 2018).
        # Maps (visit_index within loop span) -> (model_dim,) additive signal,
        # so the model can distinguish iterations through shared-weight layers.
        # Only used for layers in [loop_start, loop_end] when looping_active.
        # Store the loop range + iter count so forward paths can find the embed
        # without threading extra args through the block signature.
        self.ts_embed_enabled = h.ts_embed_enabled and h.num_loops > 0
        self.loop_start_idx = h.loop_start if h.num_loops > 0 else -1
        self.loop_end_idx = h.loop_end if h.num_loops > 0 else -1
        if self.ts_embed_enabled:
            # num_loops + 1 total visits (iter 0 is the original pass, iter N
            # is the Nth extra loop). Zero-init -> model matches v1 at step 0.
            # Name contains "ts_embed" which we add to CONTROL_TENSOR_NAME_PATTERNS
            # so this stays fp32 and goes through the scalar/control AdamW path
            # rather than matrix GPTQ.
            self.ts_embed = nn.Parameter(
                torch.zeros(h.num_loops + 1, h.model_dim, dtype=torch.float32)
            )
        else:
            self.ts_embed = None
        if h.num_loops > 0:
            ...
            # Precompute per-position visit index for ts_embed lookup. Layers
            # outside [loop_start, loop_end] get visit 0 (no ts_embed).
            # Layers inside the loop range get visit 0 for the original pass
            # and visit 1..num_loops for the extra loop iterations, counted
            # across the full enc+dec sequence.
            visit_counts = {}
            all_visits = []
            for i in all_indices:
                if h.loop_start <= i <= h.loop_end:
                    all_visits.append(visit_counts.get(i, 0))
                    visit_counts[i] = visit_counts.get(i, 0) + 1
                else:
                    all_visits.append(0)
            self.encoder_visits = all_visits[:num_enc]
            self.decoder_visits = all_visits[num_enc:]
        else:
            self.encoder_indices = list(range(self.num_encoder_layers))
            self.decoder_indices = list(range(self.num_encoder_layers, h.num_layers))
            self.encoder_visits = [0] * len(self.encoder_indices)
            self.decoder_visits = [0] * len(self.decoder_indices)
    ...
    def _apply_ts_embed(self, x, layer_idx, visit_idx):
        """Add the UT-style timestep embed if enabled and layer is in the loop range.
        No-op when ts_embed_enabled=False or when not in the loop range."""
        if not self.ts_embed_enabled or self.ts_embed is None:
            return x
        if not (self.loop_start_idx <= layer_idx <= self.loop_end_idx):
            return x
        # visit_idx is a Python int from the precomputed visits list. Safe for
        # torch.compile to index a parameter with a compile-time constant.
        return x + self.ts_embed[visit_idx].to(dtype=x.dtype)[None, None, :]
    ...
    def forward_logits(self, input_ids, cu_seqlens=None, max_seqlen=0):
        ...
        # Visit indices for ts_embed lookup. Non-loop layers get visit 0.
        enc_visits_iter = (
            self.encoder_visits if self.looping_active else [0] * self.num_encoder_layers
        )
        dec_visits_iter = (
            self.decoder_visits if self.looping_active else [0] * self.num_decoder_layers
        )
        for vi, i in zip(enc_visits_iter, enc_iter):
            q_w, k_w, v_w, out_w, up_w, down_w = self._bank_weights(i)
            x = self._apply_ts_embed(x, i, vi)
            x = self.blocks[i](x, x0, q_w, k_w, v_w, out_w, up_w, down_w, cu_seqlens=cu_seqlens, max_seqlen=max_seqlen)
            skips.append(x)
        psl = self.parallel_start_layer
        lane0 = None
        lane1 = None
        for skip_idx, (vi, i) in enumerate(zip(dec_visits_iter, dec_iter)):
            q_w, k_w, v_w, out_w, up_w, down_w = self._bank_weights(i)
            if i >= psl and psl > 0:
                if lane0 is None:
                    lane0 = x
                ...
                if skip_idx < self.num_skip_weights and skips:
                    ...
                    if self.skip_gates is not None:
                        ...
                        lane0 = lane0 + w * skip
                lane0 = self._apply_ts_embed(lane0, i, vi)
                lane1 = self._apply_ts_embed(lane1, i, vi)
    ...
    def forward_ttt(self, input_ids, target_ids, lora):
        ...
        # Visit indices for ts_embed in the TTT path.
        enc_visits_iter = (
            self.encoder_visits if self.looping_active else [0] * self.num_encoder_layers
        )
        dec_visits_iter = (
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,skip_gates,parallel_post_lambdas,parallel_resid_lambdas,attn_gate_proj,attn_gate_w,smear_gate,smear_lambda,ts_embed",
    ).split(",")
    if pattern
)
...
class Optimizers:
    def __init__(self, h, base_model):
        ...
        # Universal-Transformer timestep embed also lives on GPT root.
        if getattr(base_model, "ts_embed", None) is not None:
            scalar_params.append(base_model.ts_embed)
