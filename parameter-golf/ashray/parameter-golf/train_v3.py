...
class Hyperparameters:
    ...
    # HybridNorm (Kim et al., NeurIPS 2025, arXiv:2503.04598). Two independent
    # toggles that together comprise the full paper recipe:
    #   V_NORM_ENABLED=1         -> RMSNorm applied to the V projection inside
    #                               attention (Q and K already normed in rank 4)
    #   POST_NORM_FFN_ENABLED=1  -> replace Pre-Norm FFN with Post-Norm:
    #                               RMSNorm(x + scale * FFN(x)) instead of
    #                               x + scale * FFN(Norm(x))
    # Both off = rank 4 baseline behavior. Zero new parameters in either mode.
    v_norm_enabled = bool(int(os.environ.get("V_NORM_ENABLED", "0")))
    post_norm_ffn_enabled = bool(int(os.environ.get("POST_NORM_FFN_ENABLED", "0")))
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        # HybridNorm: V-norm toggle. When True, applies F.rms_norm to V in
        # CausalSelfAttention.forward (matches what's already done for Q and K).
        self.v_norm = v_norm
    ...
    def forward(self, x, q_w, k_w, v_w, out_w, cu_seqlens=None, max_seqlen=0):
        ...
        # HybridNorm: optionally normalize V too (Q and K are always normed above).
        if self.v_norm:
...
class Block(nn.Module):
    def __init__(
        ...
        # HybridNorm: post-norm on the FFN residual. When True, the FFN sublayer
        # runs WITHOUT the pre-norm on its input, and the full residual
        # (x_out + scale * FFN(x_out)) gets RMSNorm'd before returning.
        self.post_norm_ffn = post_norm_ffn
    ...
    def forward(self, x, x0, q_w, k_w, v_w, out_w, up_w, down_w, cu_seqlens=None, max_seqlen=0):
        ...
        if self.post_norm_ffn:
            # HybridNorm FFN: post-norm, no pre-norm on FFN input.
            mlp_in = x_out * self.ln_scale_factor
            x_out = F.rms_norm(
                x_out + self.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * self.mlp(mlp_in, up_w, down_w),
                (x_out.size(-1),),
            )
        else:
...
class GPT(nn.Module):
    def __init__(self, h):
        ...
        self.v_norm_enabled = h.v_norm_enabled
        self.post_norm_ffn_enabled = h.post_norm_ffn_enabled
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    v_norm=h.v_norm_enabled,
                    post_norm_ffn=h.post_norm_ffn_enabled,
                )
    ...
    def _parallel_block(
        ...
        # Note: HybridNorm's post-norm FFN is NOT applied in the parallel-residual
        # path. The parallel block mixes attn/mlp via learned parallel_resid_lambdas
        # and parallel_post_lambdas, which already provides a form of output-side
        # modulation. V-norm inside attn still applies via block.attn.v_norm.
    ...
    def _block_with_lora(self, block, x, x0, lora, slot, q_w, k_w, v_w, out_w, up_w, down_w):
        ...
        q = F.rms_norm(q, (q.size(-1),))
        k = F.rms_norm(k, (k.size(-1),))
        # HybridNorm V-norm (TTT sequential path) — matches eval CausalSelfAttention.forward.
        if attn.v_norm:
            v = F.rms_norm(v, (v.size(-1),))
        ...
        if self.post_norm_ffn_enabled:
            # HybridNorm FFN (TTT path): post-norm, LoRA operates on un-normed input.
            mlp_in = x_out * block.ln_scale_factor
            mlp_out = block.mlp(mlp_in, up_w, down_w)
            if lora.mlp_loras is not None:
                mlp_out = mlp_out + lora.mlp_loras[slot](mlp_in)
            x_out = F.rms_norm(
                x_out + block.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * mlp_out,
                (x_out.size(-1),),
            )
        else:
    ...
    def _parallel_block_with_lora(
        ...
        q = F.rms_norm(q, (q.size(-1),))
        k = F.rms_norm(k, (k.size(-1),))
        # HybridNorm V-norm (TTT parallel path).
        if attn.v_norm:
            v = F.rms_norm(v, (v.size(-1),))
        cos, sin = attn.rotary(seqlen, n.device, q.dtype)
        q = apply_rotary_emb(q, cos, sin, attn.rope_dims)
