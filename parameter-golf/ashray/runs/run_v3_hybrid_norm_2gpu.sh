#!/bin/bash
#
# V3 — baseline + HybridNorm (V-norm + Post-Norm FFN) on 2×H100, 1 seed.
# Measures the delta from HybridNorm (paper #21) vs v1 baseline.
#
# Usage (from ashray/ root):
#   bash runs/run_v3_hybrid_norm_2gpu.sh
#
# To ablate either half independently, set flags directly:
#   V_NORM_ENABLED=1 POST_NORM_FFN_ENABLED=0 bash runs/run_v3_hybrid_norm_2gpu.sh
#   V_NORM_ENABLED=0 POST_NORM_FFN_ENABLED=1 bash runs/run_v3_hybrid_norm_2gpu.sh

set -uo pipefail

NGPUS="${NGPUS:-2}"
SEED="${SEED:-42}"
RUN_NAME="v3_hybrid_norm_seed${SEED}_${NGPUS}gpu"

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
ASHRAY_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
PG_DIR="$ASHRAY_ROOT/parameter-golf"
BASE_CONFIG="$ASHRAY_ROOT/runs/configs/v1_base.env"
OVERLAY_CONFIG="$ASHRAY_ROOT/runs/configs/v3_hybrid_norm.env"
TRAIN_SCRIPT="train_v3.py"
RECORDS_DIR="$PG_DIR/records/$RUN_NAME"

echo "============================================"
echo "  V3 — HybridNorm — ${NGPUS}×GPU, seed ${SEED}"
echo "  $(date)"
echo "============================================"
...
# --- Load configs: base first, then overlay (user env overrides win last) ---
source "$BASE_CONFIG"
...
echo "  V_NORM_ENABLED:        $V_NORM_ENABLED"
echo "  POST_NORM_FFN_ENABLED: $POST_NORM_FFN_ENABLED"
echo ""
...
# Delta vs v1 baseline if present.
...
if os.path.exists(v1_log):
    ...
    if v1_ttt and ttt_bpb != "?":
        ...
        print(f"  delta (v3-v1):  {delta:+.5f}  ({'BETTER' if delta < 0 else 'WORSE' if delta > 0 else 'TIE'})")
# Also compare vs v2 if present.
...
if os.path.exists(v2_log):
    ...
    if v2_ttt and ttt_bpb != "?":
        ...
        print(f"  delta (v3-v2):  {delta:+.5f}  ({'BETTER' if delta < 0 else 'WORSE' if delta > 0 else 'TIE'})")
PY
