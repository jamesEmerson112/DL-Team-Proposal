"""
train_gpt_v3_1.py — Week 1 iteration on train_gpt_v2.py

Changes vs v2:
  1. Hparam retune (env-var defaults only):
       ...
       EMBED_CLIP_SIGMAS   20.0   -> 14.0   (tighter int7 embed clip, per rank 1)
  2. Per-group GPTQ clip sigmas:
       MATRIX_CLIP_SIGMAS split into MLP_CLIP_SIGMAS (default 11.5) and
       ATTN_CLIP_SIGMAS (default 13.0). Rank 4 got +0.001 BPB from retuning
       MLP clip alone (10 -> 12). Rank 1 uses 11.5.
  3. CaseOps data-path awareness (optional, env-gated, OFF by default):
       CASEOPS_ENABLED=1 points data loaders at CaseOps-tokenized shards and
       switches validation byte accounting to read fineweb_val_bytes_*.bin
       uint16 sidecars instead of the SentencePiece-LUT byte count. When
       off, behavior is identical to v2. Actual CaseOps transform +
       data-prep script land alongside this file separately.
  4. Removed dead ETLB scaffolding (eval_val_sliding_etlb was never defined in
     v2; the call path would have crashed if ETLB_ENABLED=1 was ever set).

Deliberately left for v3_2 (week 2):
  * Phased LoRA TTT (replacing current full-model score-first TTT)
  * SmearGate + BOS fix in the forward
  * Polar Express Newton-Schulz coefficients (replacing fixed Muon tuple)
  * Sparse attention head-output gate
  * Fused softcapped CE Triton kernel
  * LQER asymmetric int4 quantization
...
class Hyperparameters:
    ...
    warmdown_frac = float(os.environ.get('WARMDOWN_FRAC', .85))          # [v3.1] 0.72 -> 0.85
    ...
    # --- optimizer ---
    min_lr = float(os.environ.get('MIN_LR', .10))                         # [v3.1] 0.0 -> 0.10
    ...
    beta2 = float(os.environ.get('BETA2', .99))                           # [v3.1] 0.95 -> 0.99
    ...
    # --- test-time training (score-first; phased LoRA TTT lands in v3_2) ---
    ...
    # --- gated attention (our technique) ---
    # 'none' | 'headwise' | 'elementwise'.
    # NOTE: elementwise busts 16MB budget at MODEL_DIM=512 (17.2 MB in F3/F9).
    # Use 'headwise' for in-budget runs; v3_2 will swap this for the sparse
    # head-output gate (gate_window=12, ~96 params/layer).
    gated_attn = os.environ.get('GATED_ATTN', 'none')

    # --- value residual (ResFormer) ---
    # CONFIRMED NEGATIVE on rank-1 stack: F4 vs F1 +0.0025 BPB (2xH100), A2 vs
    # A1 +0.0022 BPB (8xH100). Parallel residuals already provide the gradient
    # highway ResFormer tries to create. Default OFF; kept for ablation only.
    ...
    # [v3.1] per-group clip sigmas.
    # Old v2 used a single MATRIX_CLIP_SIGMAS for all matrices. Rank 4 found
    # that retuning MLP alone (10 -> 12) is worth ~0.001 BPB, and rank 1
    # converges at 11.5. Attention and embedding sigmas are tuned separately.
    # MATRIX_CLIP_SIGMAS is preserved as a legacy fallback when a per-group
    # value is not explicitly set.
    _legacy_matrix_clip = float(os.environ.get('MATRIX_CLIP_SIGMAS', 12.85))
    matrix_clip_sigmas = _legacy_matrix_clip                              # legacy fallback
    mlp_clip_sigmas = float(os.environ.get('MLP_CLIP_SIGMAS', 11.5))      # [v3.1] rank 1 value
    attn_clip_sigmas = float(os.environ.get('ATTN_CLIP_SIGMAS', 13.0))    # [v3.1] rank 3/4 value
    embed_clip_sigmas = float(os.environ.get('EMBED_CLIP_SIGMAS', 14.0))  # [v3.1] 20.0 -> 14.0

    # --- CaseOps (off by default; actual transform + data-prep in a separate file) ---
    # When enabled, training + val shards come from a CaseOps-tokenized dataset
    # and validation byte accounting uses the uint16 byte sidecar emitted by
    # the prep script (fineweb_val_bytes_*.bin) instead of the SentencePiece
    # LUT. When off, behavior is identical to v2 (uses the stock SP8192 LUT).
    caseops_enabled = bool(int(os.environ.get('CASEOPS_ENABLED', '0')))
    caseops_data_dir = os.environ.get('CASEOPS_DATA_DIR', '')
    caseops_tokenizer_path = os.environ.get('CASEOPS_TOKENIZER_PATH', '')
    ...
    # --- derived paths (CaseOps-aware) ---
    if caseops_enabled and caseops_data_dir:
        datasets_dir = caseops_data_dir
    else:
        datasets_dir = os.path.join(data_dir, 'datasets', f"fineweb10B_sp{vocab_size}")
    train_files = os.path.join(datasets_dir, 'fineweb_train_*.bin')
    val_files = os.path.join(datasets_dir, 'fineweb_val_*.bin')
    val_bytes_files = os.path.join(datasets_dir, 'fineweb_val_bytes_*.bin')  # [v3.1] CaseOps sidecar
    if caseops_enabled and caseops_tokenizer_path:
...
def set_logging_hparams(h):
    global _logger_hparams; _logger_hparams = h
...
class ValidationData:
    """Loads validation tokens and builds byte-accounting tables.

    v3.1 change: when CaseOps is enabled and fineweb_val_bytes_*.bin sidecar
    files exist, load the true per-token byte counts from those files instead
    of building a LUT from the SentencePiece vocabulary. This is required for
    rule-compliant BPB when the tokenizer folds case (the post-transform
    SentencePiece piece does not equal the original UTF-8 bytes).

    Falls back to the v2 LUT path when CaseOps is off.
    """
    ...
    def __init__(self, h, device):
        ...
        self.caseops_bytes_enabled = False
        self.val_token_bytes = None  # per-token uint16 byte count, aligned with val_tokens
        if h.caseops_enabled:
            sidecar_files = sorted(glob.glob(h.val_bytes_files))
            if sidecar_files:
                self.val_token_bytes = _load_byte_sidecars(sidecar_files, self.val_tokens.numel())
                self.caseops_bytes_enabled = True
                log(f"caseops:loaded per-token byte sidecar ({len(sidecar_files)} files, {self.val_token_bytes.numel()} tokens)")
            else:
                log(f"caseops:WARNING CASEOPS_ENABLED=1 but no byte sidecar found at {h.val_bytes_files}; falling back to SP LUT")

        # Build SP LUT as a fallback / for non-CaseOps runs.
        self.base_bytes_lut, self.has_leading_space_lut, self.is_boundary_token_lut = \
            build_sentencepiece_luts(self.sp, h.vocab_size, device)

        if self.val_token_bytes is not None:
            self.val_token_bytes = self.val_token_bytes.to(device)


def _load_byte_sidecars(files, expected_n_tokens):
    """Concatenate uint16 byte-count sidecar shards. Each entry gives the
    original-UTF-8 byte count of the corresponding val token."""
    tensors = []
    for f in files:
        arr = np.fromfile(f, dtype='<u2')
        tensors.append(torch.from_numpy(arr.astype(np.int16, copy=False)))
    cat = torch.cat(tensors).contiguous()
    if cat.numel() < expected_n_tokens:
        raise ValueError(f"byte sidecar shorter than val tokens: {cat.numel()} < {expected_n_tokens}")
    return cat[:expected_n_tokens]
...
# =============================================================================
# Model (unchanged from v2 — structural changes land in v3_2)
# =============================================================================
...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim, num_heads, num_kv_heads, rope_base, qk_gain_init, train_seq_len, gated_attn='none'):
        ...
        self.head_dim = dim // num_heads; self.gated_attn = gated_attn
        ...
        self.c_q = CastedLinear(dim, dim + self.gate_dim, bias=False)
    ...
    def forward(self, x, v0=None, vr_alpha=0.0):
        ...
        if gate_logits is not None:
            if self.gated_attn == 'headwise':
                gate = torch.sigmoid(gate_logits).unsqueeze(-1); y = y * gate
            elif self.gated_attn == 'elementwise':
                gate = torch.sigmoid(gate_logits).reshape(bsz, seqlen, self.num_heads, self.head_dim); y = y * gate
        y = y.reshape(bsz, seqlen, dim); out = self.proj(y)
        if cache_v:
            return out, v_before_repeat
        return out
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init, train_seq_len,
                 layer_idx=0, ln_scale=False, gated_attn='none'):
        super().__init__()
        self.attn_norm = RMSNorm(); self.mlp_norm = RMSNorm()
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init, train_seq_len, gated_attn=gated_attn)
    ...
    def forward(self, x, x0, v0=None, vr_alpha=0.0):
        ...
        attn_result = self.attn(self.attn_norm(x_in) * self.ln_scale_factor, v0=v0, vr_alpha=vr_alpha)
        if isinstance(attn_result, tuple):
            attn_out, v_cached = attn_result
        else:
            attn_out = attn_result; v_cached = None
        if self.parallel:
...
class GPT(nn.Module):
    def __init__(self, h):
        ...
        self.value_residual_alpha = h.value_residual_alpha
        self.blocks = nn.ModuleList([
            Block(h.model_dim, h.num_heads, h.num_kv_heads, h.mlp_mult, h.rope_base, h.qk_gain_init,
                  h.train_seq_len, layer_idx=i, ln_scale=h.ln_scale, gated_attn=h.gated_attn)
    ...
    def forward_logits(self, input_ids):
        ...
        if self.embed_proj is not None:
            x = self.embed_proj(x)
        x0 = x; skips = []
        vr_alpha = self.value_residual_alpha; v0 = None
...
def zeropower_via_newtonschulz5(G, steps=10, eps=1e-07):
    # Fixed Muon NS coefficients. v3_2 will swap this for per-iter
    # Polar Express minimax tuples.
...
class Optimizers:
    def __init__(self, h, base_model):
        ...
        self.optimizer_tok = torch.optim.AdamW(tok_params, betas=(h.beta1, h.beta2), eps=h.adam_eps,
...
# =============================================================================
# GPTQ (v3.1: per-group clip sigmas)
# =============================================================================
...
def _pick_clip_sigmas(name, h):
    """[v3.1] Per-group clip sigmas. Embeddings use embed_clip_sigmas; MLP
    rows use mlp_clip_sigmas; attention rows use attn_clip_sigmas. Falls back
    to matrix_clip_sigmas for any uncategorized tensor so behavior remains
    well-defined."""
    if 'tok_emb' in name:
...
def gptq_mixed_quantize(state_dict, hessians, h):
    ...
    for (name, tensor) in state_dict.items():
        ...
        if not t.is_floating_point() or t.numel() <= 65536:
            ...
            continue
        cs = _pick_clip_sigmas(name, h)  # [v3.1] per-group clip sigmas
        bits = h.embed_bits if 'tok_emb' in name else h.matrix_bits
        q, s = gptq_quantize_weight(t, hessians[name], clip_sigmas=cs, clip_range=2 ** (bits - 1) - 1)
        result[name + '.q'] = q
        result[name + '.scale'] = s
        meta[name] = f"gptq (int{bits}, clip_sigmas={cs:.2f})"
    ...
    for cat in sorted(categories):
        log("  %s: %s" % (cat, ", ".join(sorted(categories[cat]))))
    return result, meta
...
def _byte_shuffle(data, stride=2):
    ...
    n = len(src); out = np.empty(n, dtype=np.uint8); dest_off = 0
...
def _tgt_byte_count(val_data, prev_ids, tgt_ids, dtype=torch.int16):
    """[v3.1] Resolve byte counts for a flattened target slice.

    If the CaseOps byte sidecar was loaded, the per-token count lives in
    val_data.val_token_bytes (already on-device) and we need to index it by
    the absolute val_tokens offset. Because we hand callers a slice and not
    absolute indices, we rebuild bytes from the sidecar by looking up target
    IDs directly in a secondary path; for the LUT path we keep v2's rule of
    `base_bytes[tgt] + has_leading_space[tgt] & ~is_boundary[prev]`."""
    if val_data.caseops_bytes_enabled:
        # Sidecar is addressed by val_tokens index, but callers pass token IDs.
        # The sidecar path is used inside eval_val* below where we have the
        # absolute offset available, so this helper is only used in the
        # token-ID-only code path (SP LUT).
        raise NotImplementedError("sidecar byte lookup is done inline in eval_val* with absolute offsets")
    tb = val_data.base_bytes_lut[tgt_ids].to(dtype=dtype)
    tb += (val_data.has_leading_space_lut[tgt_ids] & ~val_data.is_boundary_token_lut[prev_ids]).to(dtype=dtype)
    return tb
...
def eval_val(h, device, val_data, model):
    ...
    if local_batch_tokens < seq_len:
        raise ValueError(f"VAL_BATCH_SIZE must provide at least one sequence per rank; "
    ...
    use_sidecar = val_data.caseops_bytes_enabled
    model.eval()
    with torch.inference_mode():
        for batch_seq_start in range(seq_start, seq_end, local_batch_seqs):
            ...
            if use_sidecar:
                # Byte count for target positions: tokens at absolute offsets [raw_start+1, raw_end).
                tb = val_data.val_token_bytes[raw_start + 1:raw_end].to(torch.float64)
    ...
    with torch.inference_mode():
        for bi in range(0, len(my_windows), batch_seqs):
            ...
            for (i, ws) in enumerate(batch_ws):
                ...
                if use_sidecar:
                    tb = val_data.val_token_bytes[ws + 1 + s:ws + 1 + wlen].to(torch.float64)
                else:
...
def eval_val_ttt(h, device, val_data, base_model, batch_seqs=32):
    """Full-model score-first TTT (kept for fallback; phased LoRA TTT in v3_2)."""
...
def train_model(h, device, val_data):
    ...
    def lr_mul(frac):
        # [v3.1] warmdown with LR floor (MIN_LR). At frac >= 1 - warmdown_frac,
        # LR is linearly decayed from 1.0 -> min_lr (not 0.0 as in v2).
        if h.warmdown_frac <= 0:
            return 1.
        if frac >= 1. - h.warmdown_frac:
            decayed = (1. - frac) / h.warmdown_frac
            return max(decayed, h.min_lr)
        return 1.
...
def train_and_eval(h, device):
    ...
    _n_shards = len(list(Path(h.datasets_dir).resolve().glob('fineweb_train_*.bin')))
    log(f"train_shards: {_n_shards}")
    log(f"val_tokens: {val_data.val_tokens.numel() - 1}")
    if val_data.caseops_bytes_enabled:
        log("bpb:using CaseOps per-token byte sidecar")
    else:
        log("bpb:using SentencePiece LUT")
