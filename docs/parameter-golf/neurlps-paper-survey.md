...
| 15 | **Small Batch Size Training / Why Gradient Accumulation is Wasteful** | NeurIPS 2025 | [neurips](https://neurips.cc/virtual/2025/poster/119899) | Small batch sizes are stable with proper Adam hyperparameter scaling; avoiding gradient accumulation improves per-FLOP performance | **Tested — SUCCESS** | **Session 15-16:** Set GRAD_ACCUM_STEPS=1 + TRAIN_BATCH_TOKENS=196608 (÷4). Result: −0.015 BPB (B2=1.1419 vs C6=1.1572). 3,349 steps vs ~1,030. Biggest V2 technique win besides PreQuantTTT. Beta2 scaling (0.95→0.99) makes no difference. |
| 21 | **HybridNorm** | NeurIPS 2025 | [arxiv](https://arxiv.org/abs/2503.04598) | QKV normalization inside attention + Post-Norm in FFN; combines Pre-Norm stability with Post-Norm performance | Not tested (paper read) | Cheap (just extra norm ops). Could improve training stability and final loss at 17M scale. Easy to implement: add RMSNorm to Q,K,V projections. |
| 19 | **Differential Attention (Diff Transformer)** | ICLR 2025 (Oral) | [arxiv](https://arxiv.org/abs/2410.05258) | Computes attention as the difference of two softmax maps; cancels noise, promotes sparse attention | Not tested (paper read) | Lightweight (split Q/K heads, two softmax, subtract), minimal extra params. Noise-cancellation could improve BPB by helping the model attend more precisely. |
...
### 3. Early Weight Averaging / EMA — Paper #25 ✅ NOW IN USE
...
**V1 stack (our train_gpt.py, Run 11 = 1.2077 BPB):**
- **GQA (Grouped Query Attention)** — 8H/4KV, reduces KV parameters
- **RoPE** — standard positional encoding
- **RMSNorm** — efficient normalization
- **LeakyReLU²** — activation function (upgraded from ReLU²)
- **Muon optimizer** — second-order optimizer, ~1.4x faster than AdamW
- **int8 + zlib compression** — for the 16 MB artifact constraint
- **Gated Attention (headwise)** — our original technique, +37K params
...
- ~~**Muon -> NorMuon upgrade**~~ — now using MuonEq-R (row-normalized Muon variant, V2 stack)
- ~~**Exclusive Self-Attention (#26)**~~ — now in use (V2 stack, all 11 layers)
