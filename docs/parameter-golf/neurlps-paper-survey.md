...
| 1 | **Not All Tokens Are What You Need (Rho-1)** | NeurIPS 2024 Best Paper Runner-Up | [arxiv](https://arxiv.org/abs/2404.07965) | Selective Language Modeling — score tokens with a reference model, train only on high-excess-loss tokens | Not tested | Could skip low-information tokens in FineWeb batches, getting more "learning per step." Needs a small reference model (the baseline itself after a few steps could serve). Potentially large BPB improvement for same compute. |
